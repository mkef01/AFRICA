    ...
    function ($scope, $http, $q, $compile, $sce) {
        ...
        $scope.modificar = function ($val) {
            ...
            if ($scope.nummodificado < 1) {
                var el = '<input class="btn btn-dark" style="background-color: darkslategray; border-color: transparent" type="button" value="Modificar Item" id="mod" name="agregar" ng-click="modificarbase()" ng-model="myVal" />';
        ...
        $scope.insertar = function () {
            ...
            if ($scope.numinsertado < 1) {
                var el = '<input class="btn btn-dark" style="background-color: darkgoldenrod; border-color: transparent" type="button" id="ins" value="Modificar Item" name="agregar" ng-click="insertarbase()" ng-model="myVal" />';
                var compilado = $compile(el)($scope);
                $(compilado).appendTo('#destino');
                $scope.numinsertado = 1;
            }
        }

        $scope.insertarbase = function () {
            var a = $('#fecha').val();
            var datos = {
                "tabla": "productos",
                "campos": "nombre, descripcion, stock_minimo, tipo, producto_unidades, fecha_ultima_compra",
                "valores": "'" + $scope.nombre + "' ,'" + $scope.descripcion + "' ," + $scope.minimo + " ,'" + $scope.tipo + "' ," + $scope.unidad + " ,'" + a + "'"
            };
            console.log(datos);
            $http.post("API_GET_INSERT.php", JSON.stringify(datos)).then(function (response) {
                console.log(response.data.datosgeneral[0]);
                if (response.data.datosgeneral[0] === 1) {
                    $("#enchulame").modal("hide");
                    $scope.upate = "Insertado Exitosamente";
                    $("#loadMe").modal({
                        show: true
                    });
                    var datos = {
